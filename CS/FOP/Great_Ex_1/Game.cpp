...
void print_field(int curIns)
{
	...
	for (int i = 0; i < s_Space; i++)
		if (_Space[i])
			for (int j = 0;; j++)
			{
				...
				maxSize_space = (j == maxSize_space && Space[i] < 0) ? maxSize_space + 1 : maxSize_space;
	...
	if (current_Box)
		for (int i = 0;; i++)
		{
			maxSize_cur = (i > maxSize_cur) ? i : maxSize_cur;
			maxSize_cur = (i == maxSize_cur && currentBox < 0) ? maxSize_cur + 1 : maxSize_cur;
	...
	for (int in = p_input, out = 0, k = 0; k < MAX(s_input - p_input, s_userOutput); in++, out++, k++)
	{
		if (in < s_input)
		{
			...
			blank(maxSize_in + 4);
		blank(GAP + s_Space * (maxSize_space + 4) + GAP);
		if (out < s_userOutput)
		{
			cout << '*';
			_line(maxSize_out + 2);
...
void simulation()
{
	...
	s_userOutput = 0;
	while (instruction <= N && !gameFinish)
	{
		int p_operation = 0;
		while (p_operation < s_operation && userInput[instruction] != operation[p_operation])
		{
			p_operation++;
		}
		if (p_operation >= s_operation)
		{
			cout << "Error on instruction " << instruction << endl;
			return;
		}
	...
	while (p_output < s_output)
	{
		if (p_userOutput >= s_userOutput)
		{
			cout << "Fail" << endl;
			return;
		}
